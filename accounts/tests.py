...
class UserCreateAPIViewTest(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user_data = {'username': 'newuser', 'email': 'newuser@example.com', 'password': 'testpass123'}
...
class UserRetrieveUpdateDestroyAPIViewTest(TestCase):
    ...
    def test_user_update_api_view(self):
        ...
        response = self.client.patch(reverse('account_rud', kwargs={'pk': self.user.id}), {'username': 'John'})
        print(response.json)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_user_destroy_api_view(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.delete(reverse('account_rud', kwargs={'pk': self.user.id}))
